...
exports.getQuestionById = async function(request, response) {
    ...
    try {
        ...
        if (data && data.length) {
            ...
            return BaseController.fillResponse(
                response,
                APICodes.SUCCESS,
            ...
            console.log(
                "questionController getQuestionById question not found"
            );
            return BaseController.fillResponse(
                response,
                APICodes.NOT_FOUND,
...
exports.getQuestionsByIdWithAllAnswers = async function(request, response) {
    try {
        let data = await QuestionService.getQuestionsByIdWithAllAnswers(
            request.params.id
        );
        if (data.question.length) {
            console.log(
                "questionController getQuestionByIdWithAllComments",
                data
            );
            return BaseController.fillResponse(
                response,
                APICodes.SUCCESS,
                "Question with comments successfully retrieved",
                data
            );
        } else {
            console.log(
                "questionController getQuestionByIdWithAllComments question not found"
            );
            return BaseController.fillResponse(
                response,
                APICodes.NOT_FOUND,
...
exports.deleteAll = async function(request, response) {
    try {
        ...
        return BaseController.fillResponse(
            response,
            APICodes.SUCCESS,
            "Questions successfully deleted from database"
        );
    } catch (e) {
